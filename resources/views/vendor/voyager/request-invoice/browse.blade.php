                                            ...
                                            @foreach($dataType->browseRows as $row)
                                                ...
                                                <td>
                                                    ...
                                                    @elseif(($row->type == 'select_dropdown' || $row->type == 'radio_btn') && property_exists($row->details, 'options'))
                                                        ...
                                                        @if ( $row->field == 'status' )
                                                            @if ( $data->status == 0 )
                                                            <div class="ms-4 badge bg-label-info">En attente de confirmation</div>
...
@section('javascript')
    ...
    <script>
        ...
        $('td').on('click', '.delete', function (e) {
            $('#delete_form')[0].action = '{{ route('request-invoice.reject', '__id') }}'.replace('__id', $(this).data('id'));
            $('#delete_modal').modal('show');
        });

        $('td').on('click', '.confirmOrder', function (e) {
            $('#confirm_form')[0].action = '{{ route('request-invoice.confirm', '__id') }}'.replace('__id', $(this).data('id'));
