...
<div class="row row-dd row-dd-relationship">
    <div class="col-xs-2">
        ...
        <strong>{{ __('database.type') }}:</strong> <span>{{ __('database.relationship.relationship') }}</span><br/>
        <strong>{{ __('generic.required') }}:</strong>
    ...
    <div class="col-xs-2">
        ...
        <label for="field_browse_{{ $relationship['field'] }}"> {{ __('database.relationship.browse') }}</label><br>
        ...
        <label for="field_read_{{ $relationship['field'] }}"> {{ __('database.relationship.read') }}</label><br>
        ...
        <label for="field_edit_{{ $relationship['field'] }}"> {{ __('database.relationship.edit') }}</label><br>
        ...
        <label for="field_add_{{ $relationship['field'] }}"> {{ __('database.relationship.add') }}</label><br>
        ...
        <label for="field_delete_{{ $relationship['field'] }}"> {{ __('database.relationship.delete') }}</label><br>
    </div>
    <div class="col-xs-2">
        <p>{{ __('database.relationship.relationship') }}</p>
    </div>
    <div class="col-xs-2">
    ...
    <div class="col-xs-4">
        <div class="voyager-relationship-details-btn">
            ...
            <span class="open_text">{{ __('database.relationship.open') }}</span>
            <span class="close_text">{{ __('database.relationship.close') }}</span>
            {{ __('database.relationship.relationship_details') }}
        </div>
    </div>
    <div class="col-md-12 voyager-relationship-details">
        <a href="{{ route('voyager.bread.delete_relationship', $relationship['id']) }}" class="delete_relationship"><i class="voyager-trash"></i> {{ __('database.relationship.delete') }}</a>
        <div class="relationship_details_content">
            ...
            <select class="relationship_type select2" name="relationship_type_{{ $relationship['field'] }}">
                <option value="hasOne" @if(isset($relationshipDetails->type) && $relationshipDetails->type == 'hasOne') selected="selected" @endif>{{ __('database.relationship.has_one') }}</option>
                <option value="hasMany" @if(isset($relationshipDetails->type) && $relationshipDetails->type == 'hasMany') selected="selected" @endif>{{ __('database.relationship.has_many') }}</option>
                <option value="belongsTo" @if(isset($relationshipDetails->type) && $relationshipDetails->type == 'belongsTo') selected="selected" @endif>{{ __('database.relationship.belongs_to') }}</option>
                <option value="belongsToMany" @if(isset($relationshipDetails->type) && $relationshipDetails->type == 'belongsToMany') selected="selected" @endif>{{ __('database.relationship.belongs_to_many') }}</option>
            </select>
            ...
            <span><input type="text" class="form-control" name="relationship_model_{{ $relationship['field'] }}" placeholder="{{ __('database.relationship.namespace') }}" value="{{ $relationshipDetails->model ?? '' }}"></span>
        </div>
            <div class="relationshipField">
                <div class="relationship_details_content margin_top belongsTo @if($relationshipDetails->type == 'belongsTo') flexed @endif">
                    <label>{{ __('database.relationship.which_column_from') }} <span>{{ \Illuminate\Support\Str::singular(ucfirst($table)) }}</span> {{ __('database.relationship.is_used_to_reference') }} <span class="label_table_name"></span>?</label>
                ...
                <div class="relationship_details_content margin_top hasOneMany @if($relationshipDetails->type == 'hasOne' || $relationshipDetails->type == 'hasMany') flexed @endif">
                    <label>{{ __('database.relationship.which_column_from') }} <span class="label_table_name"></span> {{ __('database.relationship.is_used_to_reference') }} <span>{{ \Illuminate\Support\Str::singular(ucfirst($table)) }}</span>?</label>
        ...
        <div class="relationship_details_content margin_top relationshipPivot @if($relationshipDetails->type == 'belongsToMany') visible @endif">
            <label>{{ __('database.relationship.pivot_table') }}:</label>
        ...
        <div class="relationship_details_content margin_top">
            <label>{{ __('database.relationship.display_the') }} <span class="label_table_name"></span></label>
            ...
            <div class="belongsToShow belongsToManyShow relationship_details_content" style="flex:1">
                <label class="relationship_key">{{ __('database.relationship.store_the') }} <span class="label_table_name"></span></label>
            ...
            <div class="hasOneShow hasManyShow relationship_details_content" style="flex:1">
                <label class="relationship_key">{{ __('database.relationship.store_the') }}
            ...
            @isset($relationshipDetails->taggable)
                ...
                <span class="relationship_taggable">
                    <input type="checkbox" name="relationship_taggable_{{ $relationship['field'] }}" class="toggleswitch" data-on="{{ __('generic.yes') }}" data-off="{{ __('generic.no') }}" {{$relationshipDetails->taggable == 'on' ? 'checked' : ''}}>
                </span>
            @endisset
        </div>
        <div class="relationship_details_content margin_top">
            <div class="col-xs-12" style="margin: 0px !important; padding: 0px !important;">
                ...
                <label>{{ __('database.relationship.relationship_details') }}</label>
