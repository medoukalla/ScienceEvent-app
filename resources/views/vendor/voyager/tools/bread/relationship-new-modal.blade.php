	...
	<div class="offcanvas offcanvas-end" tabindex="-1" id="modalRelationship" aria-labelledby="modalRelationshipLabel">
		<div class="offcanvas-header">
			<h5 id="modalRelationshipLabel" class="offcanvas-title">
				...
				{{ __('database.relationship.relationships') }}
			</h5>
		...
		<div class="offcanvas-body my-auto mx-0 flex-grow-0">
			...
			@if(isset($dataType->id))
				<div class="col-md-12 relationship_details">
					...
					<span><input type="text" class="form-control" name="relationship_model" placeholder="{{ __('database.relationship.namespace') }}" value="{{ $relationshipDetails->model ?? ''}}"></span>
				</div>
				...
				<div class="col-md-12 relationship_details_more">
					<div class="well">
						<label>{{ __('database.relationship.selection_details') }}</label>
						<p><strong>{{ __('database.relationship.display_the') }} <span class="label_table_name"></span>: </strong>
						...
						<p class="relationship_key belongsToShow belongsToManyShow"><strong>{{ __('database.relationship.store_the') }}
						...
						<p class="relationship_key hasOneShow hasManyShow"><strong>{{ __('database.relationship.store_the') }}
						...
						<p class="relationship_taggable"><strong>{{ __('database.relationship.allow_tagging') }}:</strong> <br>
							<input type="checkbox" name="relationship_taggable" class="toggleswitch" data-on="{{ __('generic.yes') }}" data-off="{{ __('generic.no') }}">
						</p>
					</div>
				</div>
			@else
				<div class="col-md-12">
					<h5><i class="voyager-rum-1"></i> {{ __('database.relationship.easy_there') }}</h5>
