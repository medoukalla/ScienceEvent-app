@section('database-table-helper-buttons-template')
    <div>
        <div class="btn btn-success" @click="addNewColumn">+ {{ __('database.add_new_column') }}</div>
        <div class="btn btn-success" @click="addTimestamps">+ {{ __('database.add_timestamps') }}</div>
        <div class="btn btn-success" @click="addSoftDeletes">+ {{ __('database.add_softdeletes') }}</div>
    </div>
@endsection
