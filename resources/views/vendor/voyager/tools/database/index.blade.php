...
@section('page_header')
    <h1 class="page-title">
        ...
        <a href="{{ route('voyager.database.create') }}" class="btn btn-success"><i class="voyager-plus"></i>
            {{ __('database.create_new_table') }}</a>
    </h1>
@stop
                ...
                <table class="table table-striped database-tables">
                    <thead>
                        <tr>
                            ...
                            <th style="text-align:right" colspan="2">{{ __('database.table_actions') }}</th>
                        </tr>
                    </thead>
                    ...
                    <tr>
                            ...
                            @if($table->dataTypeId)
                                <a href="{{ route('voyager.' . $table->slug . '.index') }}"
                                   class="btn-sm btn-warning browse_bread">
                                    <i class="voyager-plus"></i> {{ __('database.browse_bread') }}
                                </a>
                        ...
                        <td class="actions">
                            ...
                            <a href="{{ route('voyager.database.show', $table->prefix.$table->name) }}"
                               ...
                               <i class="voyager-eye"></i> {{ __('generic.view') }}
                            </a>
                        </td>
                    </tr>
                @endforeach
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <form action="#" id="delete_table_form" method="POST">
                        {{ method_field('DELETE') }}
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        <input type="submit" class="btn btn-danger pull-right" value="{{ __('database.delete_table_confirm') }}">
                        <button type="button" class="btn btn-outline pull-right" style="margin-right:10px;"
                                data-dismiss="modal">{{ __('generic.cancel') }}
                        </button>
                    </form>
        ...
        $(function () {
            ...
            $('td.actions').on('click', '.delete_table', function (e) {
                ...
                if ($(this).hasClass('remove-bread-warning')) {
                    toastr.warning('{{ __('database.delete_bread_before_table') }}');
                } else {
